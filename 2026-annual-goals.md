...
목표 수준
미완성 항목을 마무리하고 QA를 통해 Master Templates v1.0을 안정적으로 배포한 상태.
...
목표 수준
AI 생성 결과물을 Studio 편집 경험과 연결하는 End-to-End UX 방향을 정의하고, Design Principles·Visual Identity 문서 초안을 Wiki에 장착한 상태.
...
목표 수준
AI 생성 결과물을 편집 가능한 구조로 고도화하고 핵심 UI 패턴 비교를 통해 Studio 공통 UX 가이드를 정립한 상태.
...
목표 수준
AI 생성·편집 연계 UX와 Studio Design System 운영 기준이 정착되고 Wiki·PMR·QA 루틴이 팀 프로세스로 고정된 상태.
...
- AI 결과물을 페이지·섹션·컴포넌트 단위로 편집 가능한 구조로 정의
- 생성 경험과 기존 편집 경험이 충돌하지 않도록 진입/수정 흐름 정리
- 선택 영역 재수정, 전/후 비교, 되돌리기 등 핵심 인터랙션 기준 수립
- 운영 자산으로 재활용 가능한 템플릿/컴포넌트 연결 기준 마련
...
| 구분 | 체크 항목 | 확인 기준 | 증빙 |
| --- | --- | --- | --- |
| UX 플로우 | 생성 → 편집 → 적용 흐름이 자연스럽게 이어지는가 | 핵심 시나리오 기준 점검 | 시나리오/QA 로그 |
| 편집 가능성 | 생성 결과를 주요 단위(페이지·섹션·컴포넌트)에서 수정 가능한가 | 수정/저장/재오픈 확인 | 화면 캡처 |
| UX 일관성 | Studio 핵심 UI 패턴이 가이드와 일치하는가 | 패턴 기준 대조 | Wiki/Figma 링크 |
| 문서·PMR 연계 | Wiki와 PMR 링크가 최신 상태로 연결되었는가 | No.10/17/23 링크 확인 | PMR 제출본 |
| 운영성 | 생성 결과를 운영 자산으로 재활용할 수 있는가 | 재사용 시나리오 확인 | 운영 가이드 |
